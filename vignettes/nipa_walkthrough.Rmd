...
$$ 
 T^C_{t} = \sum_{j=1}^{3} \sum_{i=0}^{11} MPC_{j,t-i} (T_{j,t-i} -\tilde{T}_{j,t-i}) 
$$
...
$$
(\frac{C_{t}}{C_{t-1}} - (1+\pi_{C})) \frac{C_{t-1}}{Y_{t-1}}
$$
...
$$
 FIM^T_{t} = (\frac{C_{t}}{C_{t-1}} - (1+\pi_{C})) \frac{C_{t-1}}{Y_{t-1}} - (\frac{C_{t} - T^C_{t}}{C_{t-1}} - (1+\pi_{C})) \frac{C_{t-1}}{Y_{t-1}}
$$
...
gt_tbl <- mpc %>%     rename_with(.fn = ~ to_title_case(.),
  ...
  pivot_longer(-Quarter, names_to = 'Components')  %>% 
  mutate(Quarter = glue::glue('Q{Quarter}')) %>% 
  pivot_wider(names_from = 'Quarter',
              values_from = 'value') %>% 
  rename(Total = QTotal) %>% 
  gt(rowname_col = 'components') 
...
library('gt')
library('scales')
 df <- read_xlsx('data/us_iip.xlsx', sheet = 'debt_stacked') %>%
  mutate(debt = if_else(type == '(% of GDP)',
                        glue('{debt * 100}%'),
                        str_remove(glue('-${comma(round(debt))}'), '(.[0]{1,})$')),
         credit = if_else(type == '(% of GDP)',
                          glue('{credit * 100}%'),
                           str_remove(glue('${comma(credit)}'),'(.[0]{1,})$' )))
    ...
    df %>% 
      ...
      group_by(type) %>% 
      gt() %>% 
      cols_label(debtor_country = '',
                 creditor_country = '',
                 debt = '',
                 credit = '') %>% 
      tab_spanner(label = 'Largest debtors',
                  columns = c('debtor_country', 'debt')) %>% 
      tab_spanner(label = 'Largest creditors',
                  columns = c('creditor_country', 'credit')) %>% 

      tab_header(title = md('Largest external creditors and debtors'),
                  subtitle = md('End of 2019')) %>% 
          tab_source_note('Note: economies with GDP above $100 billion in 2019.') %>% 
      tab_source_note('Source: Philip Lane and Gian Maria Milesi-Ferretti, the External Wealth of Nations database') %>% 
