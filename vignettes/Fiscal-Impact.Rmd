...
recession_shade <- 
    geom_rect(data = fim::recessions, aes(xmin = start, xmax = end, ymin=-Inf, ymax=+Inf),
              fill = 'grey', alpha = 0.3)
...
recession_shade <-
  geom_rect(data = recessions, aes(xmin = recession_start, xmax = recession_end, ymin=-Inf, ymax=+Inf),
...
fim %>% 
  select(date, fiscal_impact, fiscal_impact_moving_average) %>% pivot_longer(fiscal_impact) %>% 
    ggplot() +
