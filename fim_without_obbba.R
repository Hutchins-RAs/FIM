# fim_without_obbba.R
#
# This script runs the FIM without the effects of the OBBBA. This modifies
# parts of the FIM inputs, notably: federal purchases, medicaid, student
# loans, SNAP (social benefits), personal income taxes, corporate taxes,
...
federal_purchases_obbba <- tribble(
  ~date, ~data_series,
  "2025 Q3", 19,
  "2025 Q4", 30,
  "2026 Q1", 45,
...
# Remove OBBBA add factors from federal purchases
federal_purchases_test <- federal_purchases_test %>%
  left_join(federal_purchases_obbba, by = "date", suffix = c("", "_obbba")) %>%
  mutate(data_series = if_else(is.na(data_series_obbba),
                                data_series,
                                data_series - data_series_obbba)) %>%
  select(-data_series_obbba) %>%
  as_tsibble(index = date)
