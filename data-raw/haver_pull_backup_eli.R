...
wla_imputed<-bind_rows(wla, wla_currentq)
                                                                
#wla_imputed<-wla%>% mutate_where(date == "2022-12-31",yptolm=wla_currentq$yptolm)
ctc_imputed<-bind_rows(ctc, ctc_currentq)
...
state_ui_imputed<-state_ui%>% mutate_where(date == "2022-12-31",licl=state_ui_currentq$licl,
  ...
  write_xlsx(state_ui_imputed, 'data/monthly_state_ui.xlsx')
  
usna_imputed<-bind_rows(usna, usna_currentq)%>%
