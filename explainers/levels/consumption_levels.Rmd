...
totals %>% 
  group_by(category) %>% 
  summarise(contribution_data = list(total), .groups = 'drop') %>% 
  gt() %>% 
  gt_theme_nytimes() %>% 
  gt_kable_sparkline(contribution_data)
```
...
transfers %>% 
  ...
  geom_line(data = . %>% filter(date > yearquarter("2021 Q2"), mapping = aes(lty = "dashed line")))
