...
-   The guides and checklists in the data/[forecast.xlsx]{.file} workbook.
...
We host the FIM at the following Github repository: <https://github.com/Hutchins-Center/fim>. It contains the code, data, and output for the FIM. It was originally authored by Manuel Alcala Kovalski (Manu).
...
3.  Clone the following repository to a folder on your computer: <https://github.com/Hutchins-Center/fim>. (In the Desktop App, go to `File`\>`Clone a respository`\>`URL` and paste in the link.) In `Local path` choose a location on your computer where you'd like to host the `fim` folder. <i><u>Note</i></u>: Choose a file path that is not synced to Sharepoint. More specifically, the file should be on your hard drive, not one that is uploaded to a Cloud.

We use Git to [push]{.git-object} our own changes and [pull]{.git-object} other contributors' changes to the FIM.

-   Any updates you make within the `fim` folder will have to be saved using [commit]{.git-object} in the Github Desktop app. Make sure to use a descriptive commit summary that highlights the changes you made.

-   After committing your changes, you will have to [push]{.git-object} your changes to [fetch]{.origin}(<https://github.com/Hutchins-Center/fim>) to make them available to other contributors.

-   Pushed changes can then be [fetch]{.git-object}-ed and [pull]{.git-object}-ed within the Github Desktop app by others.

<i><u>Note</i></u> that merging changes for the same file can be tricky so multiple users should not be simultaneously working on the same file. Wait for them to [push]{.git-object} their changes. Then [fetch]{.git-object} and [pull]{.git-object} them in before making changes to the same file.

Only approved contributors can make changes to the repository. To be added, please reach out to the admin with your Github username. The current admin is Lorae Stojanovic (`@lorae-brookings`). To add contributors, admins can navigate to <https://github.com/Hutchins-Center/fim/settings/access> and add people by their username. They can also remove contributors or reassign admin duties. For security, try to restrict the admin designation to one RA at a time.
:::

::: {#section-1-2}
### Install the [fim]{.r-object} package

After cloning the `fim` repository to your computer, you should install the [fim]{.r-object} package in R. This package, written by Manu, contains all functions required for the FIM. Again, it is highly recommended that you consult Manu's [read-me](https://github.com/Hutchins-Center/fim#readme) and the resources that he cites to better understand how the FIM works.

1.  Install RTools from the Software Center if you don't already have it.

2.  Install the [fim]{.r-object} package by running the following code in your R console:
...
3.  <a id = "load_fim"></a> To load the [fim]{.r-object} package, open the [fim.Rproj]{.file} file in the fim directory. This will set the working directory to the fim folder. <u>IMPORTANT: Make sure to always have the [fim.Rproj]{.file} file open before running any code related to the FIM.</u>

4.  Next, open the [fiscal_impact.R]{.file} script and run up till the line `devtools::load_all()` (line 8 at the time of writing this guide) to load the [fim]{.r-object} package and all relevant functions. <i><u>Note</i></u>: if it is your first time running the script, it is best to run this section one line at a time and install all dependencies. Each subsequent time, you can load the [fim]{.r-object} by pressing `CTRL + SHIFT + L` when working within the [fim.Rproj]{.file} file.
...
-   Used for 'pulling in the data' when GDP numbers come in, used every update.<a id = "haver_pull"></a>
    ...
    -   It updates the [national accounts]{.r-object} dataset and the <i>Haver Pivoted</i> sheet in the data/[forecast.xlsx]{.file} workbook.
...
-   We call this the <a id = "forecast"></a>'forecast sheet' (even though it's a workbook and has a sheet called <i>forecast</i> within it), used every update.
    ...
    -   When new BEA data comes in, we use data-raw/[haver_pull.R]{.file} to pull it into the <i>Haver Pivoted</i> sheet. The <i>Haver Pivoted</i> sheet contains quarterly BEA data up to and including the most recent numbers in the right-most column.
...
::: {#section-3-2-3}
#### [fiscal_impact.R]{.file} {#fiscal_impact.r}

-   <a id = "fiscal_impact.R"></a>What we call 'running the FIM', used every update.

    -   This is the main script that calculates the FIM. Typically, this is the only script we run after we are happy with the [forecast.xlsx]{.file} sheet.
    -   It pulls in the updated data (the [national accounts]{.r-object} and [projections]{.r-object} datasets), the updated *forecast* sheet from the data/[forecast.xlsx]{.file} workbook, the historical overrides and our updated deflators (*historical overrides*, and *deflators_override* sheets from the same workbook).
    -   It cleans, reshapes, and defines the FIM categories for the *historical* and *forecast* period by calling all the relevant functions from the [fim]{.r-object} package.
    -   It calculates the FIM by generating the counterfactual levels, getting the difference from the actuals, applying MPCs, and their % contributions to GDP. (See the methodology document linked [above](#%3Ci%3E%3Cu%3Enote%3C/i%3E%3C/u%3E) for details)
    -   It generates the update comparison (where we compare how much our estimates have changed since the last update by category), reported in the [index.html]{.file} file.
    -   It generates the web materials that we publish on the website: `results/month-year/`[Fiscal-Impact-month-year.html]{.file} and `results/month-year/`[interactive-month-year.csv]{.file}. It also saves a copy of the [forecast.xlsx]{.file} workbook for the current update in the same `results` directory for future reference.

    </div>

::: {#section-3-2-4}
#### inst/extdata/[projections.xlsx]{.file}
...
::: {#section-3-2-5}
#### data-raw/[projections.R]{.file}

-   Used only when CBO has a new baseline.
    -   This script pulls CBO data into R.
    -   We run it after we've updated inst/extdata/[projections.xlsx]{.file} with a new CBO baseline.
    -   It pulls the *economic* and *budget* sheets from [projections.xlsx]{.file}, cleans them, and updates the [projections]{.r-object} dataset.
    -   The updated [projections]{.r-object} dataset is used when we run the FIM using [fiscal_impact.R]{.file}. This means that we only have to run [projections.R]{.file} once we get a new CBO baseline.
    -   <i><u>Note</i></u> that neither [projections.xlsx]{.file} nor [projections.R]{.file} updates the [forecast.xlsx]{.file} workbook following a CBO release: they only update the [projections]{.r-object} dataset. We manually paste in certain series from the CBO update into the forecast sheet (see instructions here)
:::
:::
...
2.  <a id = "pull"></a> Pull GDP data
    ...
    -   <i><u>Note</i></u>: We pull data from three Haver databases: <b> USNA</b>, <b>USECON</b> and <b>CPIDATA</b>. Each variable that we pull has a haver code associated with it. If you want to add a new variable to the FIM, you should add the haver code and variable name to data/[haver_names.xlsx]{.file} (if it's in <b> USNA</b>) or you should add it to the data-raw/[haver_pull.R](#haver_pull) script using the `pull_data()` function. Make sure to put this new variable at the end of the [national accounts]{.r-object} dataset using the `relocate()` function so as to not mess up the formulas in the forecast sheet.
...
5.  <a id = "publish"></a> Publish results
    ...
    -   Publishing on [Wordpress](https://www.brookings.edu/wp-admin/):
        ...
        -   In the left-hand panel, select Content \> Articles
        -   Search "fiscal impact" in the search bar. The Hutchins Center Fiscal Impact Measure post should come up. Hit "Edit".
        -   Update the text of the post in the WYSIWYG tab to reflect the latest write up. Make sure to not get rid of any of the html code on this page while doing so.
        -   On the left sidebar, navigate to the "Sidebar / Widgets" tab. In the file labeled "Fiscal impact," upload the `Fiscal-Impact.pdf` that you saved from the html version. To do this, hover over the paper icon and click on the pencil icon to edit. Upload the new file, making sure that you select the "Replace the file, use the new file name, and update all links" and "Replace the date with the current date" options.
        -   To upload the interactive file, you will need to upload the CSV file to the Media Library. Click on the icon for the uploaded file in the Media Library grid. In the Attachment details window, copy the File URL to your clipboard. Go under the Interactives tab from the left sidebar and choose "Hutchins Center Fiscal Impact Measure CSV Update." Replace the https link with the URL for the new CSV file. It should now reflect this update's month and year. (The file path that you're replacing should look something like this: [`https://www.brookings.edu/wp-content/uploads/2023/04/interactive-05-2023.csv`](https://www.brookings.edu/wp-content/uploads/2023/04/interactive-05-2023.csv)) Click on the Update button in the right-hand column of the template.
        -   You should now be able to replace the interactive file in the "Sidebar / Widgets" tab using the same steps as replacing the pdf. Note that the file name will most probably include a "-1" near the end to indicate that a CSV file has already been saved with that name in the Media Library.
    ...
    -   Publishing on [Hubspot](https://app.hubspot.com/login/):
        ...
        -   Select the chart. Hit "Replace" in the righthand panel. Upload the screenshot of the main fim graph (the pink one) from the `Fiscal-Impact.pdf` file (the same one you pasted in the write up.) Update the Alt text if needed and make sure the Link (not optional for us) is set to <https://www.brookings.edu/interactives/hutchins-center-fiscal-impact-measure/>.
        ...
        -   Make sure everything looks good in the email that you receive. Forward a copy of it David, Louise and Stephanie. Once they sign off, forward the draft to Comms (at the time of writing this guide, we email Ashley Schelling at `AWood@brookings.edu`,) asking them to schedule for it go out on Monday.
...
-   <a id = "baseline"></a> As mentioned in the [data](#data) section, we get routine updates from the CBO that inform our baseline forecast. Here are the steps to incorporate a new CBO baseline (you can also use the [forecast.xlsx]{.file} file's *Checklist (CBO Budget Release)* if you prefer):
...
###### 1. Updating the [projections]{.r-object} dataset

-   Update the *economic* and *budget* sheets within `inst/extdata/`[projections.xlsx]{.file} workbook using the instructions in the *dictionary* sheet. When copying and pasting new projections, do not include data beyond 9/30/2034, as the subsequent code is designed to process data only up to that quarter. Read the documentation files in `docs/technical_methodology/_book/`, especially the *economics-methodology* section, for more information on addressing this issue in 2032 (when the inclusion of the eight-quarter forecast period exceeds the number of data columns).
-   Make sure to save your work.
-   Run `data-raw/`[projections.R]{.file}. This overwrites the [projections]{.r-object} dataset.
-   You can check whether the update has worked by [loading](#load_fim) the [fim]{.r-object} package and running `View(projections)` in your console. Git [commit]{.git-object} and [push]{.git-object} your changes once you feel confident about your work.

###### 2. Updating the [forecast.xlsx]{.file} workbook

-   In the *forecast comparison* sheet of [forecast.xlsx]{.file}, copy the <u>values</u> in the Current Forecast table, and paste the <u>values</u> into the Previous Forecast table. Label the table headings to reflect the date of the Previous Forecast and the date of the CBO update.
