...
![](images/minus_neutral_purchases.png)
...
![](images/scale_to_gdp_purchases.png)
...
### Taxes and Transfers FIM Function 
...
Lastly, we generate the previously mentioned "Manu file" - the update comparison file which compared this month's results to last month's. 
...
1. `scripts/index_temp.R` :  Runs the code to generate tables and figures.
...
```{r r-chunk-22, eval=FALSE, echo=TRUE}
...
2. `update-comparison-markdown.Rmd` : Pulls the figures from `index_temp.R` to generate an html file.
...
3. `results/{month_year}/beta/update-comparison-{month_year}` : Displays the results in beautiful html file. 
