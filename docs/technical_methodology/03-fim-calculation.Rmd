...
In the helper script, we define several unit functions, which are the specific steps we apply to our input data to eventually produce our outputs - the contribution of each policy category to the overall FIM. We produce a contribution for federal purchases, state purchases, and consumption (taxes and transfers). The overall FIM is simply the sum of these contributions.
...
*So what gets an MPC?* 
In summary, we apply MPCs to all of our taxes and transfers policies except for supply side IRA. We do not apply MPCs to purchases or grants.
...
minus_neutral_t <- function(x, # our data series
                            c, # consumption
                            counterfactual # counterfactual consumption
) {
...
Now that we've defined all of our functions, we need to apply them to our data. 
...
Next, we calculate the FIM for net transfers by simply applying each of our consumption functions: 
...
Note: I'm now realizing we just apply the transformations directly in this script rather than using the functions we defined in the helper script. We should fix that but I'm too lazy rn. 
...
1. We store our inputs and outputs as Excel sheets. We often review this data with Louise. We also sometimes need this data in later months when we want to compare our results with a prior publication. 2. We generate the materials that are needed to publish the FIM on the Brookings website. This includes a .csv file that populates the interactive graph on the FIM webpage and the pdf document displaying a static image of the FIM that is also available on the website. 
...
```{r section-section-C.6-output-results, eval=FALSE, echo=TRUE}
# This code block prints the chunk entitled "section-C.4-calculate-purchases-fim" 
# from fiscal_impact_BETA.R
```
