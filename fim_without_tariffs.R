# fim_without_tariffs.R
#
# This script runs the FIM without the effects of the tariffs and tariff 
# uncertainty. This modifies parts of the FIM inputs, notably: tariff uncertainty
# and federal non-corporate taxes
...
file_copy(
  ...
  new_path = glue('results/{month_year}/input_data/forecast_{month_year}_without_tariffs.xlsx'), 
  overwrite = TRUE
)
...
# Note: federal non corporate taxes is the sum of personal income, social 
# insurance, and production taxes. 

# Create sum of personal income and social insurance taxes from forecast sheet
personal_income_social_insurance_test <- readxl::read_xlsx('data/forecast.xlsx', 
                                                      sheet = 'Taxes') %>%
    slice(c(5, 6, 9, 12)) %>%
    select(-c(1:2)) %>%
    t() %>%
    as_tibble(.name_repair = "minimal") %>%
    setNames(c("year", "quarter", "personal_income", "social_insurance")) %>%
    fill(year) %>%
    filter(!is.na(quarter)) %>%
    mutate(
      date = yearquarter(paste(year, quarter)),
      data_series = as.numeric(personal_income) + as.numeric(social_insurance)
    ) %>%
    select(date, data_series) %>%
    filter(!is.na(data_series)) 

# Pull production taxes and projections from June 2025 forecast sheet to estimate
# production tax revenue minus tariffs 
production_test <- readxl::read_xlsx('results/06-2025/input_data/forecast_06-2025.xlsx', 
                                     sheet = 'Taxes') %>%
  slice(c(5, 6, 13)) %>%
  select(-c(1:2)) %>%
  t() %>%
  as_tibble(.name_repair = "minimal") %>%
  setNames(c("year", "quarter", "production")) %>%
  fill(year) %>%
  filter(!is.na(quarter)) %>%
  mutate(
    date = yearquarter(paste(year, quarter)),
    data_series = as.numeric(production)
  ) %>%
  select(date, data_series) %>%
  filter(!is.na(data_series)) 

# Pull historical data on federal non corporate taxes 
federal_non_corporate_taxes_history <- federal_non_corporate_taxes_test %>%
  filter(date >= yearquarter("1970 Q1") & date <= yearquarter("2019 Q1"))

# Sum components of federal non corporate taxes and merge with history 
federal_non_corporate_taxes_test <- data.frame(
  date = personal_income_social_insurance_test$date, 
  data_series = personal_income_social_insurance_test$data_series + 
    production_test$data_series
) %>%
  coalesce_join(create_placeholder_nas(), by = 'date') %>%
  mutate(across(everything(), ~ replace_na(., 0))) %>% 
  rbind(federal_non_corporate_taxes_history) %>%
  arrange(date) 
...
openxlsx::write.xlsx(contributions_df, file = glue('results/{month_year}/beta/contributions-{month_year}_without_tariffs.xlsx'), overwrite = TRUE)
openxlsx::write.xlsx(inputs_df, file = glue('results/{month_year}/beta/inputs-{month_year}_without_tariffs.xlsx'), overwrite = TRUE)
...
readr::write_csv(interactive,  file = glue('results/{month_year}/beta/interactive-{month_year}_without_tariffs.csv'))
...
file_copy(path = 'Fiscal-Impact.html',
          new_path = glue('results/{month_year}/beta/Fiscal-Impact-{month_year}_without_tariffs.html'),
          overwrite = TRUE)
